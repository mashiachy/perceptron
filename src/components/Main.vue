<template>
  <teleport to="header">
    <h1>
      {{ curTabComponent === 'DetectLetter' ? 'Распознавание буквы' : 'Получение матричного представления' }}
    </h1>
  </teleport>
  <main class="main">
...
<script>
  ...
  export default {
    ...
    setup () {
      const curTabComponent = ref('GetMatrix');
      const curTabComponentName = computed(() => `tab${curTabComponent.value}`);
